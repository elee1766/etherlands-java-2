...
public class ChatService extends ListenerClient {
  ...
  private void gamer_district_info(Gamer gamer, District district) {
    ...
    if(clusters != null){
      builder.addField("locations",ComponentCreator.Clusters(clusters));
    }
    if(district.hasTown()){
      ...
      builder.addBody("team permissions",ComponentCreator.TeamPermissions(district.getTown(),district.getIdInt(),all_teams.stream().toList()));
...
//    }
  ...
  private void gamer_team_info(Gamer gamer, Team team) {
    ...
    if(!team.getName().equals("outsider")){
      builder.addField("members",ComponentCreator.Gamers(team.getMembers()));
    }
  ...
  private void gamer_town_info(Gamer gamer, Town target) {
    ...
    if(!target.exists()){
      TextComponent message = ComponentCreator.ColoredText("town does not exist",ChatColor.YELLOW);
      channels.chat_message.publish(new Message<>(ChatTarget.gamer, gamer, message));
      return;
    }
    MessageCreator builder = new MessageCreator();
  ...
  private void send_global(String message, Gamer arg){
    ...
    name.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,"/gamer "+arg.getPlayer().getName()));
    ...
    if(arg.hasTown()){
      ...
      town.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,"/town "+arg.getTown()));
  ...
  private void send_town(Town town, String message, Gamer arg){
    ...
    prefix.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,"/town "+town.getName()));
    TextComponent name = new TextComponent(arg.getPlayer().getName());
    name.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,new Text("[Click to see info]")));
    name.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,"/gamer "+arg.getPlayer().getName()));
    name.setColor(ChatColor.WHITE);
    ...
    for (Gamer member: town.getMembers()) {
      Player player = Bukkit.getServer().getPlayer(member.getUuid());
      if(player != null){
