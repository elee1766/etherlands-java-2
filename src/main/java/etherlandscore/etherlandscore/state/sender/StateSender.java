...
public class StateSender {
  ...
  public static void addFriend(Channels channels, Gamer self, Gamer gamer) {
    channels.master_command.publish(
        new Message<>(MasterCommand.gamer_add_friend, self, gamer)
            .setChatResponse(ChatTarget.gamer_add_friend_response, self, gamer));
  }
  ...
  public static void gamerFailAction(Channels channels, PermissionedAction action) {
    channels.chat_message.publish(new Message<>(ChatTarget.gamer_fail_action, action));
  }
  ...
  public static void removeMember(Channels channels, Gamer gamer, Town town) {
    channels.master_command.publish(new Message<>(MasterCommand.town_remove_gamer, town, gamer));
  }
  ...
  public static void setAddress(Channels channels, UUID uuid, String address) {
    channels.master_command.publish(new Message<>(MasterCommand.gamer_link_address, uuid, address));
  }

  public static void setGamerPermission(
      Channels channels, Gamer gamer, AccessFlags flag, FlagValue value, District district) {
    channels.master_command.publish(
        new Message<>(MasterCommand.district_set_gamer_permission, district, gamer, flag, value)
            .setChatResponse(ChatTarget.gamer_district_info, gamer, district));
  }

  public static void setMessageToggle(
      Channels channels, MessageToggles flag, ToggleValues value, Gamer gamer) {
    channels.master_command.publish(
        new Message<>(MasterCommand.gamer_toggle_message, gamer, flag, value)
            .setChatResponse(
  ...
  public static void setTeamPermission(
      ...
      Gamer gamer) {
    channels.master_command.publish(
        new Message<>(MasterCommand.district_set_team_permission, district, team, flag, value)
            .setChatResponse(ChatTarget.gamer_district_info, gamer, district));
  }
